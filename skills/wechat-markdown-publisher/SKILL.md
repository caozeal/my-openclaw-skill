...
```bash
python3 ./skills/wechat-markdown-publisher/scripts/md_to_wechat.py <input.md> --theme wechat --embed-images --copy
```
...
- 图片语法兼容普通 Markdown `![alt](path)` 与 Obsidian `![[path]]`
- 基础表格支持
...
- 默认命令按 **workspace 根目录** 作为当前目录来写；如果你不在 workspace 根目录执行，请改成正确相对路径或绝对路径。
